package de.kp.works.stream.sql.transform.tls
...
/**
 * The [Fleet] and [TLS] based beats send the same
 * harmonized event messages.
 *
...
object TLSSchema {
  /**
   * This method determines the TLS schema from the
  ...
  def fromTable(table:String):StructType = {
    ...
    val format = TLSTablesUtil.fromTable(table)
    ...
    try {
       ...
       * log events.
       */
      if (format == TLSTables.OSQUERY_STATUS)
        TLSSchema.osquery_status()
