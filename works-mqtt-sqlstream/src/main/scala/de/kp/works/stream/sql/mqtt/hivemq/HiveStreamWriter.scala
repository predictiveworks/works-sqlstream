...
class HiveStreamWriter(
  ...
  override def abort(epochId: Long, writerCommitMessages: Array[WriterCommitMessage]): Unit = {
    log.info(s"epoch $epochId of HiveStreamWriter aborted.")
  }
  /**
  ...
  override def commit(epochId: Long, writerCommitMessages: Array[WriterCommitMessage]): Unit = {
    log.info(s"epoch $epochId of HiveStreamWriter committed.")
  }

  override def createWriterFactory(): DataWriterFactory[InternalRow] =
    HiveStreamWriterFactory(options, outputMode, schema)

}

/**
 * A [DataWriterFactory] for HiveMQ writing. This factory
 * will be serialized and sent to executors to generate the
 * per-task data writers.
 */
case class HiveStreamWriterFactory(
  options:HiveOptions,
  outputMode:OutputMode,
